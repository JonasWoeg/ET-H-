...
class function():
    ...
    def find_recursive(self, coeff, mask, acc, mzr):
        ...
        while True:
            ...
            #if mask == 0: print('[{0}, {1}] {2} add = {3}'.format(str(cf[0]), str(cf[1]), str(y_1), str(add)))
            ...
            if self.constant(gradients, acc):
                add *= mzr
                #if mask == 0: print('add was doubled')
            ...
            if (y_1 - y_0) <= 0:
                ...
                add /= -mzr
                #if mask == 0: print('add was cut in half')
    ...
    def constant(self, arr, acc):
        if len(arr) >= 2:
            if abs(arr[0] - arr[1]) < acc:
                return True
        return False
...
if __name__ == '__main__':
    ...
    f.coeff = f.find_recursive(f.coeff, 0, 0.00001, 2)
